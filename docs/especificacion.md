...
- ✓ Newsletter: cuenta real `buttondown.com/trainmusiq` publicada, endpoint corregido de fetch+JS a `<form action=... target="_blank">` nativo (Buttondown exige un desafío Cloudflare Turnstile que un `fetch()` en segundo plano no puede resolver). **Verificación humana de punta a punta completada por el fundador (15 jul 2026): Turnstile superado, doble opt-in confirmado por correo, suscriptor visible en el panel de Buttondown.** Newsletter cerrado, sin pendientes.
...
## 13. Bugs de producción encontrados en Firefox + reordenamiento de modos (15 jul 2026)

El fundador probó el deploy en **Firefox** (no solo Chrome, usado hasta ahora en todas las verificaciones de esta sesión) y encontró tres fallas reales.

### 13.1 Paralelismo roto: "usando 1 núcleo" en un M1 Max de 10 núcleos

**Causa raíz:** `navigator.deviceMemory` es una API de **Chrome/Edge únicamente** — Firefox y Safari no la implementan por diseño (para reducir fingerprinting) y devuelven `undefined`. `chooseWorkerPlan()` (`engine/adaptive-workers.mjs`) asumía **4GB** cuando no había dato (`deviceMemoryGB || 4`) — con `WASM_INSTANCE_MEMORY_MB=2048` y `MAX_MEMORY_FRACTION=0.5`, esa cuenta da `4×1024×0.5/2048 = exactamente 1`. Cualquier navegador sin la API quedaba matemáticamente forzado a `nParallel=1`, sin importar cuántos núcleos reales tuviera el equipo.

**Fix:** cuando `deviceMemoryGB` no está disponible, ya no se asume un número — se confía en `hardwareConcurrency` (núcleos), que **Firefox sí reporta correctamente**. El techo de RAM solo se aplica cuando el navegador la reporta de verdad (Chrome/Edge). La red de seguridad para equipos con RAM real insuficiente es el reintento por OOM de §13.2, no una suposición pesimista por adelantado.

### 13.2 OOM en canción completa (Firefox): nunca más "reventar"

**Contexto:** el motor vendorizado tiene `INITIAL_MEMORY=2048MB` fijo, sin `ALLOW_MEMORY_GROWTH` (ver CLAUDE.md — fix aplicado en la sesión de vendorización por un bug de `TextDecoder` con memoria resizable). Confirmado en Chrome: franjas de 35s andan, 40-45s+ abortan con `Aborted(OOM)`. `MAX_SAFE_SEGMENT_SECS=34` ya incluye margen bajo ese límite — **medido en Chrome**. Otro motor JS (Firefox) puede tener menos margen real con la MISMA memoria WASM fija (glue code, tablas, stack distintos) — no hay forma de saber el número exacto sin acceso a un Firefox real, así que en vez de adivinarlo, se construyó un reintento automático.

**Fix:** `SEGMENT_SECS_FALLBACK_TIERS = [34, 30, 26]` en `adaptive-workers.mjs`. Si el motor aborta por memoria (detectado por patrón en el mensaje de error: `OOM`/`out of memory`/`memory access out of bounds`), `separate.worker.mjs` reintenta la separación completa con el siguiente nivel (franjas más chicas → más franjas → mismo paralelismo), informando el motivo real por la UI de progreso, nunca fallando en silencio ni mostrando un error críptico. Los niveles 30s/26s se eligieron para quedar bien por encima de `2×DISCARD_SECS=23.4s` — un nivel más chico que eso degenera (verificado con una simulación: `[34,24,16]` producía 274 franjas para una canción de 274s en el segundo nivel, en vez de las ~42-106 razonables de `[34,30,26]`). Un error que NO sea de memoria (un bug real) se propaga inmediatamente, sin reintentar — la lógica de reintento fue verificada con una simulación aislada (4 casos: recuperación en el segundo nivel, en el tercero, agotamiento de todos los niveles, y propagación inmediata de errores no relacionados a memoria).

**Limitación honesta:** este entorno de trabajo no tiene un Firefox real para probar — el diagnóstico de 13.1 se apoya en un hecho de plataforma bien documentado (MDN: `navigator.deviceMemory` no implementado en Firefox), y el fix de 13.2 es una red de seguridad genérica (no depende de saber el número exacto de Firefox). Verificado en Chrome (dev y deploy) y con simulación aislada de la lógica de reintento — la verificación en un Firefox real queda para el fundador.

### 13.3 "Sección de estudio" (antes "Fragmento") rota: bug real de coordenadas en la forma de onda

**Causa raíz, confirmada por reproducción directa:** el listener de arrastre sobre el `<canvas>` de la forma de onda calculaba `startSec`/`endSec` dividiendo `e.offsetX` (espacio CSS, el ancho **renderizado** del canvas) por `canvas.width` (el búfer interno de dibujo, fijo en 1200px por el atributo HTML) — dos escalas distintas. El canvas se renderiza al 100% del ancho del contenedor (`.rig { max-width: 760px }`), casi siempre bastante menos que 1200px. Resultado verificado: arrastrar sobre el 100% del ancho VISIBLE de una canción de 60s seleccionaba solo hasta el ~60% de la duración real — **la parte final de cualquier canción quedaba inalcanzable por arrastre**, exactamente el síntoma reportado ("no puedo seleccionar el coro/solo del final").

**Fix:** usar `canvas.getBoundingClientRect().width` (el ancho realmente renderizado) en vez de `canvas.width` para convertir la posición del mouse a segundos. Verificado con reproducción antes/después: un arrastre del 10%-90% del ancho visible de un clip de 60s daba `3.6s–32.5s` antes del fix (comprimido) y `6.0s–40.0s` después (correcto, con el clamp de 34s aplicándose sobre el punto de inicio correcto); un arrastre hacia el final (85%-99.9%) alcanza correctamente `51.0s–59.9s` después del fix.

**Reencuadre de producto (pedido del fundador):** el modo pasa a llamarse **"Sección de estudio"** — la parte que se está practicando (un solo, un coro, un puente) aislada con calidad bit-perfecta garantizada, en vez de "Fragmento" (nombre técnico, no dice para qué sirve). Usa el modelo de 4 stems existente por ahora; **evolucionará a 6 stems en v2.1** (guitarra/piano) para aislar solos con más precisión — ese salto de modelo es su diferenciador frente a los modos de canción completa, que se quedan en 4 stems. Identificador interno de modo renombrado de `"fragment"` a `"studio"` en `app.mjs` y `workers/separate.worker.mjs` para que el código hable el mismo idioma que el producto.
